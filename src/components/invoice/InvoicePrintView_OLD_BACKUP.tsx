...
function InvoicePrintView({ invoice, innerRef, showInternalView = false }: InvoicePrintViewProps) {
  // Calculate items per page - accounting for header, customer info, and footer space
  // Approximate: ~20 items per page for safety
  const ITEMS_PER_PAGE = 20;
  
  // Split line items into pages
  const pages: InvoiceLineItem[][] = [];
  for (let i = 0; i < invoice.lineItems.length; i += ITEMS_PER_PAGE) {
    pages.push(invoice.lineItems.slice(i, i + ITEMS_PER_PAGE));
  }
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{
            ...
            alignItems: 'center',
            justifyContent: 'flex-end',
            marginBottom: totalPages > 1 ? '8px' : '0'
          }}>
            <span style={{
              fontWeight: '500',
              fontSize: '14px'

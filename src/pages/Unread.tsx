...
import { getUnreadEmails } from '../services/emailService';
...
function Unread() {
  ...
  const fetchEmails = async (forceRefresh = false) => {
    try {
      setLoading(true);
      const data = await getUnreadEmails(forceRefresh);
      setEmails(data);
    } catch (error) {
  ...
  const handleEmailUpdate = (updatedEmail: Email) => {
    setEmails(prevEmails => 
      ...
      ).filter(email => !email.isRead) // Remove emails that are now read from the unread list
    );
  };
    ...
    return (
      <div className="fade-in">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-semibold text-gray-800">Unread</h1>
        </div>
